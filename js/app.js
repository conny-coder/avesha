(() => {
    ...
    if (document.querySelector(".sort-cards__select")) {
        const sortSelect = document.querySelector(".sort-cards__select");
        const elementsFiltering = document.querySelectorAll(".sort-cards__item");
        startFiltering(sortSelect);
        function startFiltering(sortSelect) {
            const sortSelectIndex = sortSelect.selectedIndex;
            let sortSelectElement = sortSelect.querySelectorAll("option")[sortSelectIndex];
            searchAndShowFiltering(sortSelectElement.value);
        }
        function searchAndShowFiltering(sortSelectValue) {
            if ("all" == sortSelectValue) {
                elementsFiltering.forEach((elem => {
                    elem.classList.add("active");
                }));
                return;
            }
            elementsFiltering.forEach((elem => {
                elem.classList.contains(sortSelectValue) ? elem.classList.add("active") : elem.classList.remove("active");
            }));
        }
        sortSelect.addEventListener("change", (function() {
            const sortSelectValue = sortSelect.value;
            searchAndShowFiltering(sortSelectValue);
        }));
    }
    window["FLS"] = true;
    isWebp();
